...
notification-map-bots-disabled = На этой карте отключены боты.
notification-two-humans-required = Для начала матча на этом сервере требуется не менее двух игроков-людей.
...
notification-no-start-until-required-slots-full = Невозможно начать игру, пока не заполнены все необходимые слоты.
notification-no-start-without-players = Невозможно начать игру при отсутствии игроков.
notification-insufficient-enabled-spawn-points = Невозможно начать игру, пока не будет включено больше точек спавна.
notification-malformed-command = Некорректная { $command } команда
notification-state-unchanged-ready = Невозможно изменить состояние, если готов.
notification-invalid-faction-selected = Выбрана недопустимая фракция: { $faction }
notification-supported-factions = Поддерживаемые значения: { $factions }
notification-state-unchanged-game-started = Невозможно изменить состояние при запуске игры. ({ $command })
notification-requires-host = Это может сделать только хост.
...
notification-team-player-disconnected = { $player } (Команда { $team }) отключилась.
notification-observer-disconnected = { $player } (Наблюдатель) отключился.
notification-unknown-map = Карта не найдена на сервере.
notification-searching-map = Поиск карты на Ресурсном Центре...
...
notification-you-were-kicked = Вы были выгнаны с сервера.
notification-admin-kicked = { $admin } выгнал { $player } с сервера.
notification-kicked = { $player } был выгнан с сервера.
notification-temp-ban = { $admin } временно заблокирован { $player } с сервера.
notification-admin-transfer-admin = Только администраторы могут передавать админку другому игроку.
notification-admin-move-spectators = Только хост может перемещать игроков в зрители.
notification-empty-slot = Никого в этом слоте.
notification-move-spectators = { $admin } перевел { $player } в наблюдатели.
notification-nick-changed = { $player } теперь известен как { $name }.
notification-player-dropped = Игрок был исключен из игры (timeout).
notification-connection-problems =  У { $player } возникли проблемы с подключением (timeout).
notification-timeout-dropped = { $player } был отключён (timeout).
notification-timeout-dropped-in =
    { $timeout ->
        [one] { $player } будет отключен через { $timeout } секунд.
       *[other] { $player } будут отключены через { $timeout } секунд.
    }
notification-error-game-started = Игра уже началась.
notification-requires-password = Сервер требует пароль.
notification-incorrect-password = Неправильный пароль.
notification-incompatible-mod = На сервере установлен несовместимый мод.
notification-incompatible-version = На сервере установлена несовместимая версия.
notification-incompatible-protocol = На сервере используется несовместимый протокол.
notification-you-were-banned = Вы были заблокированы на сервере.
notification-you-were-temp-banned = Вы были временно заблокированы на сервере.
notification-game-full = Игра заполнена.
notification-new-admin = { $player } теперь администратор.
notification-option-locked = { $option } не может быть изменен.
notification-invalid-configuration-command = Недопустимая команда конфигурации.
notification-admin-option = Только хост может установить этот параметр.
notification-error-number-teams = Количество команд не удалось обработать: { $raw }
notification-admin-kick = Только хост может отключить игрока.
notification-kick-self = Хосту не разрешается отключаться.
notification-kick-none = Никого в этом слоте.
notification-no-kick-game-started = Только наблюдатели и пораженные игроки могут быть выгнаны после начала игры.
notification-admin-clear-spawn = Только администраторы могут очищать точки спавна.
notification-spawn-occupied = Вы не можете занимать ту же точку спауна, что и другой игрок..
notification-spawn-locked = Точка спавна заблокирована Точка спавна привязана к другому игроку.
notification-admin-lobby-info = Только хост может изменить информацию о лобби.
notification-invalid-lobby-info = Отправлена недопустимая информация о лобби.
notification-player-color-terrain = Цвет был скорректирован, чтобы меньше походить на местность.
notification-player-color-player = Цвет был изменен, чтобы быть менее похожим на другого игрока.
notification-invalid-player-color = Невозможно определить правильный цвет игрока. Был выбран случайный цвет.
notification-invalid-error-code = Не удалось проанализировать сообщение об ошибке.
notification-master-server-connected = Связь с главным сервером установлена.
notification-master-server-error = Не удалось установить связь с главным сервером.
notification-game-offline = Игра не рекламировалась в Интернете.
notification-no-port-forward = Порт сервера недоступен из Интернета.
notification-blacklisted-server-name = Имя сервера содержит слово, занесенное в черный список.
notification-requires-authentication = Сервер требует от игроков наличия учетной записи на форуме OpenRA.
notification-no-permission-to-join = У вас нет разрешения присоединиться к этому серверу.
...
notification-game-paused = Игра приостанвлина { $player }
notification-game-unpaused = Игра продолжена { $player }

## Server
notification-game-started = Бой начинается!

## PlayerMessageTracker
notification-chat-temp-disabled =
    { $remaining ->
        [one] Чат отключен. Пожалуйста, повторите попытку через { $remaining } секунд.
       *[other] Чат отключен. Пожалуйста, повторите попытку через { $remaining } секунд.
    }

## VoteKickTracker
notification-unable-to-start-a-vote = Невозможно начать голосование.
notification-insufficient-votes-to-kick = Недостаточно голосов, чтобы выгнать игрока { $kickee }.
notification-kick-already-voted = Вы уже проголосовали.
notification-vote-kick-started = Игрок { $kicker } начал голосование, чтобы выгнать игрока { $kickee }.
notification-vote-kick-in-progress = { $percentage }% игроков проголосовали за то, чтобы выгнать игрока { $kickee }.
notification-vote-kick-ended = Голосование для изгнания { $kickee } потерпел неудачу.

## ActorEditLogic
label-duplicate-actor-id = Скопировать Actor ID
...
## SaveMapLogic
label-unpacked-map = распакованно

dialog-save-map-failed =
    .title = Не удалось сохранить карту
    .prompt = Подробности см. в файле debug.log.
    .confirm = OK

dialog-overwrite-map-failed =
    .title = Внимание
    .prompt = При сохранении вы перезапишете
    уже существующую карту.
    .confirm = Сохранить

dialog-overwrite-map-outside-edit =
    .title = Внимание
    .prompt = Карта была отредактирована вне редактора.
    При сохранении вы можете перезаписать прогресс
    .confirm = Сохранить

notification-save-current-map = Сохранение текущей карты.
...
label-mission-failed = Провалена

## GameInfoStatsLogic
label-client-state-disconnected = Ушел
label-mute-player = Отключить звук этого игрока
label-unmute-player = Включите звук этого игрока
button-kick-player = Выгнать игрока
button-vote-kick-player = Проголосуйте, чтобы выгнать этого игрока
...
dialog-vote-kick =
    .title = Голосуйте за то, чтобы выгнать { $player }?
    .prompt = Этот игрок не сможет вернуться в игру.
    .prompt-break-bots =
    { $bots ->
        [one] Отключение администратора также отключает 1 бота..
       *[other] Отключив администратора игры, вы также отключите { $bots } бота(-ов).
    }
    .vote-start = Начать голосование
    .vote-for = Голосуйте за
    .vote-against = Голосовать против
    .vote-cancel = Воздержаться

notification-vote-kick-disabled = Голосование отключено на этом сервере.

## GameTimerLogic
label-paused = Пауза
label-max-speed = Максимальная скорость
...
label-chat-availability =
    { $seconds ->
        [one] Чат будет доступен через { $seconds } секунд...
       *[other] Чат будет доступен через { $seconds } секунд...
    }
...
menu-ingame =
    ...
    .exit-map = Выход из редактора карт
